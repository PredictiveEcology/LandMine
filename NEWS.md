...
* `EstimateTruncPareto()` now calls `LandWebUtils::landmine_estimate_kBest()`. The fit is unchanged -- still Dave Andison's "95% of the area is in 5% of the fires" rule of thumb, not a fit to NBAC/NFDB -- but it is now unit-tested, and the two superseded versions of that rule, which the objective carried as commented-out code, are recorded in the function's documentation where they read as history rather than as live alternatives.
* `sim$fireSizes` is now **named by simulation year** instead of indexed by `time - burnInitialTime + 1`. Those coincide only when `fireTimestep` is 1: with a longer timestep the burn years are spaced out, the intervening list slots stay `NULL`, and `rbindlist()` drops them -- so the `idcol` this object's own documentation told users to read as the year would silently have been a 1..n counter. The metadata now says what the `year` column actually contains.
* **Mixedwood stands now use the `ROSTable`'s mixed rates** (LandWebUtils >= 1.0.3.9033). They previously fell through to `ROSother` -- the mature-spruce rate at every age -- because the `mixed` fuel type is invented for unmatched vegetation classes rather than derived from the species mapping, so the rate-of-spread join dropped it and both `mixed` rows of Andison's Table 3.2 were dead. On WesternAlbertaUpland all 126,850 mixedwood pixels move from 30 to 12 or 17. Larch is a related but distinct case and is unchanged (3 pixels there); see the LandWebUtils NEWS. **This changes simulated fire regimes.**
* The young age class's species filter is now applied unconditionally, matching the mature and immature classes. No effect where pine is present (0 of 4,458,168 WesternAlbertaUpland pixels), but on a pine-free study area every young stand previously fell through to `ROSother` instead of its `immature_young` rate.
